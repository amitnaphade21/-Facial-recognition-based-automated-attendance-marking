...
def preprocess_image(image):
    """
    Detect the largest face in the image and preprocess it for FaceNet.
    """
    rgb_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Convert to RGB for MTCNN
    detections = detector.detect_faces(rgb_image)  # Detect faces
    ...
    # Resize the face to 160x160 (FaceNet input size) and normalize pixel values
...
def augment_image(image):
    """
    Generate multiple augmented versions of the input image by rotating it.
    """
    augmented_images = []
    ...
    for angle in angles:
        h, w = image.shape[:2]
        center = (w // 2, h // 2)
        matrix = cv2.getRotationMatrix2D(center, angle, 1.0)  # Get rotation matrix
        augmented = cv2.warpAffine(image, matrix, (w, h))  # Apply rotation
...
def generate_embeddings(image):
    ...
    for aug_image in augmented_images:
        aug_image_tensor = torch.tensor(aug_image).permute(2, 0, 1).unsqueeze(0).float()  # Prepare for FaceNet
        with torch.no_grad():
            embedding = facenet(aug_image_tensor).detach().numpy().flatten()  # Get embeddings
...
np.save(r"A:\SEM 5\EDI\refined_face_embeddings.npy", embeddings)
